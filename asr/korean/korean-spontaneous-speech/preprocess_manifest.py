...
def strip_annotations(text: str) -> str:
    # Remove partially well-formed annotations
    stripped = utils.nested_sub(
        r"[(\[]+\s*\??\s*[A-Z0-9]+\s*[:;]+\s*([^\])]*)\s*[\])]+",
        r"\1",
        text,
        flags=re.IGNORECASE,
    ...
    # Remove ill-formed annotations
    stripped = utils.nested_sub(
        r"[(\[]*\s*(?:FP|SP|SN(?!S)|NO|N0)\s*[:;]?\s*([^\])]*)\s*[\])]+",
        r"\1",
        stripped,
        flags=re.IGNORECASE,
    )
    stripped = re.sub(
        r"[(\[]+\s*(?:FP|SP|SN|NO|N0)\s*[:;]+",
        r"",
        stripped,
        flags=re.IGNORECASE,
    )
    ...
    stripped = re.sub("\([숨|입]소리\)", "", stripped)
