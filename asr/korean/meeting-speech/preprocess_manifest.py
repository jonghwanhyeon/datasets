...
def normalize_transcription(text: str) -> str:
    # (@전문 용어 A)/(@전문 용어 B) -> (전문 용어 A)/(전문 용어 B)
...
def strip_annotations(text: str) -> str:
    def replacement(match: re.Match) -> str:
        phonetic, orthographic = match.group(1), match.group(2)
    ...
    stripped = utils.nested_sub(r"/\((?:noise|bgm)\)", r"", stripped, flags=re.IGNORECASE)
    stripped = re.sub(r"@(?:기침|노래|목청|박수|웃음)", r"", stripped)
    stripped = stripped.replace("(())", "")

    # (의견에 공감하는데요)/(idiom) -> 의견에 공감하는데요
    stripped = re.sub(r"\(([^()]+)\)/\(idiom\)", r"\1", stripped)
    ...
    stripped = re.sub(r"[-_=~∼·ㆍ]", r" ", stripped)

    # Replace special symbols as whitespace
    # stripped = re.sub(r"[\x08\x1b\x1f\u202c\ufeff]", r" ", stripped)
    stripped = re.sub(r"[;>\]\\]", r" ", stripped)
...
if __name__ == "__main__":
    ...
    console.log("[bold green]Saving preprocessed manifest...")
    preprocessed_manifest_path = dataset_path / "manifest-clean.parquet"
    df.to_parquet(preprocessed_manifest_path)
